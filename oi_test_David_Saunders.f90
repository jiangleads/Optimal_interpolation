	...
	integer, parameter :: n=ntheta*i_pg, on=nx*ny, m=30 !输入输出场大小, m影响半径


	real :: x(2,on), var(1), f(1,on)
...
character(len=1),parameter :: fun(2) = "g"
real,parameter ::adapt=0. !
	...
	call optiminterp(x, f, var,  param,  adapt,  fun,  m, xi, fi, vari)
	!call optiminterp (ox(nd,np), of(nf,np), ovar(np|->|nf), cor_len(1:nd), adapt, fun, m, gx, gf(nf,gn), gvar(nf,gn))

	!==============================================================================================================
	!===========================处理完毕，输出数据=====================================

	open (40,File='D:\Desktop\chazhi\v.txt',Form='formatted')
	do i = 1,i_pg*ntheta
